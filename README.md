...
- Before automatic registration, compares a separate submission gameplay hash from the loaded game chart against the server-selected official chart. Confirmed TUF metadata selects the original ZIP entry; without a confirmed selection, all ZIP charts must have one gameplay identity. Color and ordinary decoration edits are allowed, while excerpts, autoplay and gameplay edits are rejected. Existing activity hash v4 is unchanged. See [submission chart identity](docs/submission-chart-identity.md) for the contract, unsupported chart features, and rollout requirements.
