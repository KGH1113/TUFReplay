...
Published pass visuals can be edited by the submitter on the TUF pass page. Viewers can independently choose visible presets from that submitter’s library inside the player. Hiding a preset excludes it from new submissions and viewer choices and clears its defaults on published passes; showing it again does not restore old defaults. Visual changes preserve the playback clock and do not reload the level or recorded evidence. See [pass visual settings and rollout](docs/pass-visuals.md).
...
- Before accepting any evidence, compares a fresh submission gameplay hash from the loaded game chart at each play start against the server-selected official chart. Editor edits and immediate retries are checked independently. The hash covers both modern angle charts and legacy sprite charts that retain `pathData` in ADOFAI. Unverified attempts retain only a bounded local buffer and cannot create server runs or upload artifacts; only approved attempts gain a Submit action in local activity. TUF's `targetLevel` selects the original ZIP entry even when `pathConfirmed` is false; only when no target is available must all ZIP charts have one gameplay identity. Color and ordinary decoration edits are allowed, while excerpts, autoplay and gameplay edits are rejected. Final submission checks the admitted hash again. Existing activity hash v4 is unchanged. See [submission chart identity](docs/submission-chart-identity.md) for the contract, unsupported chart features, and rollout requirements.
- Registers saved Jipper Resourcepack, Jipper KeyViewer, DMNote, Impl DMNote, and ImplResourcePack visual presets through the companion web UI and authenticated mod IPC. Missing fonts/images can be attached during registration and are preserved in the preset bundle. Registered presets can be renamed without re-uploading their assets or changing replay selections; the new name also appears on existing published replays. DMNote imports the saved selected tab from multi-tab exports. Registration also accepts a separate CSS file, a fixed position and 10–400% keyviewer size on a 16:9 screen, and its global key-counter visibility setting (off by default). Submissions independently select at most one keyviewer and one overlay, with selections fixed for retries. See [the visual preset contract](docs/visual-presets-contract-2026-09-16.md) and [local E2E verification](docs/visual-presets-local-e2e-2026-09-16.md) for tested flows and remaining compatibility limits.
...
Beta releases use the same two assets and must be marked as a prerelease on GitHub. Version 0.2.0-beta.1 introduces replay engine `tufreplay.replay.v2`, payload format 1, the automatic submission pipeline, and a one-time web notice for records from the previous engine. The activity database uses application ID `0x54554652` and schema version 2, while gameplay identity uses hash v4. Existing schema v1 databases are upgraded in place to add the submission run ID column and index without losing recordings. Schema v15 activity and microphone records are imported while the source is preserved as `tufreplay.pre-0.2.sqlite`; replay payloads recorded by the previous engine are intentionally not imported. Schema v14 and earlier databases log a warning and are replaced with a fresh current database. App SemVer, activity schema, replay engine, replay payload, and gameplay hash versions are independent compatibility boundaries.
