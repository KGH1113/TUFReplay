...
입력은 기록 종료(`terminalTimeUs`)까지 보존하고 판정과 제출 키 개수는 클리어 시각(`wonTimeUs`)까지 집계한다. 음수 시간값과 CSV 값을 유지한다. 과거 lifecycle·설정·health 스트림은 재구성한 테스트 데이터다. P1 등급과 file ID도 mock 값이다. 당시 P/G 적격 여부나 과거 차트 호환성을 검증하지 않으며, 로컬 차트 사본을 테스트용 공식 ZIP으로 제공한다. 기존에 클리어 이후 입력을 잘라 만든 사본은 종료 시각을 복원해도 누락 입력까지 복구되지 않으므로, 전체 입력 검증에는 원본에서 새로 추출한 fixture를 사용한다. UI의 보완 항목에서 이 차이를 확인할 수 있다.
