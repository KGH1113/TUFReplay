  ...
  public static InstalledLevelContext Read(string levelPath, int expectedLevelId)
  {
    ...
    for (int depth = 0; directory != null && depth < 16; depth++, directory = directory.Parent)
    {
      ...
      if (!File.Exists(manifestPath))
        continue;
      if (new FileInfo(manifestPath).Length > 65536)
        throw new InvalidDataException("Invalid installation manifest.");
      var manifest = JObject.Parse(File.ReadAllText(manifestPath));
      if ((int?)manifest["Id"] != expectedLevelId)
        throw new InvalidDataException("Installation level differs.");
      ...
      foreach (char character in hash)
        if (!Uri.IsHexDigit(character))
          throw new InvalidDataException("Invalid installation hash.");
