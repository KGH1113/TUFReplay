...
public sealed class RunIssuanceClient : IDisposable
{
  ...
  )
  {
    ...
    request.Headers.Authorization = new AuthenticationHeaderValue(
      "Bearer",
      await account.AccessToken(cancellation).ConfigureAwait(false)
    );
    request.Content = new StringContent(
      JsonConvert.SerializeObject(
        ...
        {
          protocol_version = 1,
          client_game_version = gameVersion,
          client_mod_version = modVersion,
          tuf_level_id = level.LevelId,
          client_tuf_file_id = level.FileId,
    ...
    while ((count = await stream.ReadAsync(bytes, total, bytes.Length - total, cancellation).ConfigureAwait(false)) > 0)
    {
      ...
      if (total == bytes.Length)
        throw new InvalidOperationException("Oversized issuance response.");
    }
    var data = JObject.Parse(Encoding.UTF8.GetString(bytes, 0, total));
    if (
      !Guid.TryParse((string)data["run_id"], out var id)
    ...
    var leaseDuration =
      leaseDurationMs > 0 ? TimeSpan.FromMilliseconds(leaseDurationMs.Value) : expiry - DateTimeOffset.UtcNow;
    if (leaseDuration <= TimeSpan.Zero)
      throw new InvalidOperationException("Expired issuance response.");
    string expectedPath = "/api/v1/runs/" + id + "/stream";
    if ((string)data["websocket_url"] != expectedPath)
      throw new InvalidOperationException("Unexpected upload URL.");
    var websocket = new UriBuilder(account.Server)
    {
      Scheme = account.Server.Scheme == "https" ? "wss" : "ws",
      Path = expectedPath,
    };
