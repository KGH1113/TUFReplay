...
public sealed class OAuthSession : IDisposable
{
  ...
  public async Task<string> Begin()
  {
    ...
    try
    {
      if (_logout && !string.IsNullOrEmpty(_refresh))
        await RevokeAndForget().ConfigureAwait(false);
      _logout = false;
      _completedState = null;
      _completedCodeHash = null;
      _state = RandomToken();
      _verifier = RandomToken();
      ...
      return new Uri(_options.TufApi, "oauth/authorize").AbsoluteUri
        ...
        + "&scope="
        + Uri.EscapeDataString("User.Read.Public User.Submission.Create")
        + "&state="
        + _state
  ...
  public async Task Complete(string code, string state)
  {
    ...
    try
    {
      if (!_logout && !_disposed && state != null && state == _completedState && HashCode(code) == _completedCodeHash)
        return;
      if (
        _state == null
        || state != _state
        || DateTimeOffset.UtcNow >= _pendingExpires
        || _logout
      ...
      _completedCodeHash = HashCode(code);
    }
    finally
    {
      _gate.Release();
  ...
  public async Task<string> AccessToken(CancellationToken cancellation)
  {
    ...
    try
    {
      if (_disposed || _logout || string.IsNullOrEmpty(_refresh))
        throw new OAuthRejectedException();
      if (_expires <= DateTimeOffset.UtcNow.AddSeconds(60))
        await Refresh().ConfigureAwait(false);
      if (_logout)
        throw new OAuthRejectedException();
      return _access;
  ...
  {
    if (!string.IsNullOrEmpty(_refresh))
      await _http.Revoke(_refresh).ConfigureAwait(false);
    _store.Delete();
    _refresh = null;
    _access = null;
  }
  ...
  private async Task Refresh()
  {
    try
    {
      Accept(
        await _http
          .Token(new Dictionary<string, string> { ["grant_type"] = "refresh_token", ["refresh_token"] = _refresh })
          .ConfigureAwait(false)
      );
    }
  ...
  {
    string access = (string)value["access_token"],
    ...
    if (
      ...
      || refresh.Length > 2048
      || expires <= 0
      || (string)value["token_type"] != "Bearer"
    ...
    SaveCredential();
    _access = access;
    _expires = DateTimeOffset.UtcNow.AddSeconds(expires);
  ...
  private void SaveCredential() =>
    _store.Save(
      new JObject { ["refreshToken"] = _refresh, ["revocationPending"] = _logout }.ToString(
