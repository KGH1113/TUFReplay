...
public sealed class LevelChangeListener : IDisposable
{
  ...
  public LevelChangeListener(SubmissionAccount account, int levelId)
  {
    var url = new UriBuilder(account.Server)
    {
      ...
      Path = $"/api/v1/levels/{levelId}/changes",
    }.Uri;
    _ = Task.Run(async () =>
    {
      ...
      while (!_cancellation.IsCancellationRequested)
      {
        try
        {
          using var socket = new WebSocketUploadConnection(
            url,
            await account.AccessToken(_cancellation.Token).ConfigureAwait(false)
          );
          ...
          while (!_cancellation.IsCancellationRequested)
          {
            ...
            if (generation.HasValue && generation.Value != next)
