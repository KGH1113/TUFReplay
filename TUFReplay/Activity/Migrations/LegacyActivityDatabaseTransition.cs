...
public static class LegacyActivityDatabaseTransition
{
  public const int ImportVersion = 15;
  public const int MaximumDiscardedVersion = 14;
  ...
  )
  {
    ...
    if (recovery == ReplacementRecovery.RestoredBackup)
      throw new InvalidOperationException(
        "An interrupted TUFReplay 0.2 migration was rolled back. The legacy database was restored; restart to retry."
      );
    ...
    using (SqliteConnection source = Open(mainPath))
    {
      ...
      if (applicationId != 0 || version < 0 || version > ImportVersion)
        throw new InvalidOperationException(
    ...
    if (version <= MaximumDiscardedVersion)
    {
      logWarning?.Invoke(
        "[Database] Legacy activity schema v"
          + version
          + " is too old to import. Deleting the legacy activity database and starting fresh."
      );
    ...
    ImportV15(mainPath, migratingPath, backupPath);
  }

  private static void ImportV15(string mainPath, string migratingPath, string backupPath)
  {
    if (File.Exists(backupPath))
      throw new IOException("The TUFReplay pre-0.2 database backup already exists: " + backupPath);
    DeleteDatabase(migratingPath);
