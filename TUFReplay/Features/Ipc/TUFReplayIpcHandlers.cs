...
public static class TUFReplayIpcHandlers
{
  ...
  public static object GetChart(IpcRequest request) { string id=IpcParams.OptionalString(request,"id");try{ChartData x=ActivityQueryService.GetChart(id);if(x==null)return IpcDomainError.Create("level_session_not_found","Level session was not found.");if(x.levelText==null)return IpcDomainError.Create("chart_unavailable","The recorded chart path is unavailable.");return new ActivityChartDto{LevelSessionId=x.id,LevelText=x.levelText,FloorCount=x.floorCount};}catch(Exception ex){Main.Instance?.Log("[IPC] Chart read failed: "+ex.GetType().Name);return IpcDomainError.Create("chart_read_failed","The recorded chart could not be read.");} }
