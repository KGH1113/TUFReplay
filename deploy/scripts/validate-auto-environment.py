#!/usr/bin/env python3
"""Validate the private auto-submission environment without printing values."""
...
def main() -> int:
    ...
    if not path.is_file():
        print("Auto-submission environment file: missing")
        return 1

    try:
        values, duplicates = parse_env(path)
    except (OSError, UnicodeError):
        print("Auto-submission environment file: unreadable")
        return 1
    ...
    if errors:
        print("Auto-submission environment preflight failed:")
        for error in errors:
            print(f"- {error}")
        return 1

    print("Auto-submission environment preflight passed.")
    return 0
