...
import type { ActivityChart } from "@/models/activity/activity-model";
import {
  ChartEmbedClient,
  type ChartEmbedClientCallbacks,
} from "@/shared/clients/chart-embed-client";
...
function setup() {
  ...
  const callbacks = {
    ...
    onLoaded: mock(() => {}),
    onError: mock(() => {}),
    onFloorSelected: mock(() => {}),
    onMarkerSelected: mock(() => {}),
  } satisfies ChartEmbedClientCallbacks;
  const bridge = new ChartEmbedClient(frame, "https://chart.example", callbacks);
  const send = (data: object, origin = "https://chart.example", eventSource: object = source) =>
    bridge.handleMessage({
      origin,
      source: eventSource,
      data: { protocol: "web-adofai.chart", version: 1, ...data },
    } as unknown as MessageEvent);
  return { bridge, callbacks, postMessage, send, source };
}

describe("ChartEmbedClient child contract", () => {
  test("accepts chart.ready and emits exact command-specific correlation fields", () => {
    ...
    bridge.load(chart);
    bridge.setMarkers([
      { id: "floor-2", floorIndex: 2, count: 1, clearCount: 0, bestLastFloorIndex: 5 },
    ]);
    ...
    expect(load.type).toBe("chart.load");
    expect(typeof load.requestId).toBe("string");
    expect(load.revision).toBeUndefined();
    expect(markers.type).toBe("markers.set");
    expect(markers.revision).toBe(1);
    expect(markers.requestId).toBeUndefined();
    ...
    expect(postMessage.mock.calls[4]?.[0]).toEqual({
      protocol: "web-adofai.chart",
      version: 1,
      type: "run.clear",
    });
    expect(postMessage.mock.calls[5]?.[0]).toEqual({
      protocol: "web-adofai.chart",
      version: 1,
      type: "chart.clear",
    });
  ...
  test("restores the current chart and markers when the child becomes ready again", () => {
    ...
    expect(postMessage.mock.calls.map((call) => (call[0] as { type: string }).type)).toEqual([
      "chart.load",
      "markers.set",
    ]);
  });
  ...
  test("correlates loaded/error by requestId only and rejects stale or malformed loads", () => {
    ...
    bridge.setMarkers([]);
    send({ type: "chart.ready" });
    const loadMessage = postMessage.mock.calls[0]?.[0] as { requestId?: string } | undefined;
    expect(loadMessage?.requestId).toBeDefined();
    if (!loadMessage?.requestId) throw new Error("chart.load requestId was not sent");
    const requestId = loadMessage.requestId;
