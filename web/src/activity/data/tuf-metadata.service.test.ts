...
describe("TUF metadata live API contract", () => {
  ...
  test("reads song/diffId and resolves difficulty from the catalog endpoint", async () => {
    ...
    const fetcher = createFetcher(urls, {
      [`${API_ROOT}/levels/byId/3118`]: { id: 3118, song: "First Town Of This Journey", artist: "Camellia (かめりあ)", creator: "Appeal", diffId: 17 },
      [`${API_ROOT}/difficulties`]: [{ id: 17, name: "P17", icon: "https://api.tuforums.com/icon.png" }],
    });
  ...
  test("deduplicates levels and the shared difficulty catalog across concurrent requests", async () => {
    const urls: string[] = [];
    const fetcher = createFetcher(urls, {
      [`${API_ROOT}/levels/byId/3118`]: { id: 3118, song: "First", artist: "A", creator: "C", diffId: 17 },
      [`${API_ROOT}/levels/byId/5376`]: { id: 5376, song: "HALL", artist: "Frums", creator: "한가지", diffId: 21 },
      [`${API_ROOT}/difficulties`]: [{ id: 17, name: "P17", icon: "p17.png" }, { id: 21, name: "U15", icon: "u15.png" }],
    });

    const [first, duplicate, second] = await Promise.all([
      getTufMetadata(3118, fetcher), getTufMetadata(3118, fetcher), getTufMetadata(5376, fetcher),
    ]);
  ...
  test("keeps level metadata with unknown difficulty when the catalog is unavailable", async () => {
    const urls: string[] = [];
    const fetcher = createFetcher(urls, {
      [`${API_ROOT}/levels/byId/3118`]: { id: 3118, song: "First Town", artist: "Camellia", creator: "Appeal", diffId: 17 },
    });
