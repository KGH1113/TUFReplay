  ...
  return <section className="flex min-h-0 flex-1 gap-3 p-3">
    ...
    <aside className="w-80 shrink-0 overflow-y-auto rounded-lg border border-border bg-muted/10 p-3">
      ...
      {!selectedMarker ? <p className="mt-3 text-sm text-muted-foreground">Select a floor marker in the chart to inspect its runs.</p> : <>
        ...
        <div className="mt-4 space-y-2">{selectedRuns.map((run) => <div key={run.Id} className="rounded-md border border-border bg-background/60 p-2 text-xs"><div className="flex justify-between font-medium"><span>Run #{run.RunIndex}</span><span>{run.Result}</span></div><div className="mt-1 text-muted-foreground">{run.StartTile} → {run.LastTile ?? "open"} · {formatTime(run.StartedAtUtc, timeZone)}</div><div className="mt-1 text-muted-foreground">{run.InputCount} inputs · {run.HitContextCount} hits · {run.LevelPitchPercent ?? "?"}% pitch</div></div>)}</div>
      </>}
    </aside>
  </section>;
}
