...
export interface ChartEmbedMarker {
  id: string;
  floorIndex: number;
  count: number;
  clearCount: number;
...
export interface ChartEmbedClientCallbacks {
...
export class ChartEmbedClient {
  private revision = 0;
  private requestId = "";
  private ready = false;
  private chart: ChartEmbedDocument | null = null;
  private markers: ChartEmbedMarker[] = [];
  private markerSignature = "";

  constructor(
    private readonly frame: HTMLIFrameElement,
    private readonly origin: string,
    private readonly callbacks: ChartEmbedClientCallbacks,
  ) {}
  ...
  load(chart: ChartEmbedDocument) {
    this.chart = chart;
    this.sendChart();
  }

  setMarkers(markers: ChartEmbedMarker[]) {
...
function markerSetSignature(markers: readonly ChartEmbedMarker[]) {
  return markers
    .map(
      (marker) =>
